...
NSDictionary *pm25_DictionaryForNode(xmlNodePtr currentNode, NSMutableDictionary *parentResult,BOOL parentContent);
NSArray *pm25_PerformXPathQuery(xmlDocPtr doc, NSString *query);
...
NSDictionary *pm25_DictionaryForNode(xmlNodePtr currentNode, NSMutableDictionary *parentResult,BOOL parentContent)
{
    ...
    if (attribute) {
        ...
        while (attribute) {
            ...
            if (attribute->children) {
                NSDictionary *childDictionary = pm25_DictionaryForNode(attribute->children, attributeDictionary, true);
                if (childDictionary) {
    ...
    if (childNode) {
        ...
        while (childNode) {
            NSDictionary *childDictionary = pm25_DictionaryForNode(childNode, resultForNode,false);
            if (childDictionary) {
...
NSArray *pm25_PerformXPathQuery(xmlDocPtr doc, NSString *query)
{
    ...
    for (NSInteger i = 0; i < nodes->nodeNr; i++) {
        NSDictionary *nodeDictionary = pm25_DictionaryForNode(nodes->nodeTab[i], nil,false);
        if (nodeDictionary) {
...
NSArray *pm25_PerformHTMLXPathQuery(NSData *document, NSString *query) {
    return pm25_PerformHTMLXPathQueryWithEncoding(document, query, nil);
}
...
NSArray *pm25_PerformHTMLXPathQueryWithEncoding(NSData *document, NSString *query,NSString *encoding)
{
    ...
    NSArray *result = pm25_PerformXPathQuery(doc, query);
    xmlFreeDoc(doc);
...
NSArray *pm25_PerformXMLXPathQuery(NSData *document, NSString *query) {
    return pm25_PerformXMLXPathQueryWithEncoding(document, query, nil);
}
